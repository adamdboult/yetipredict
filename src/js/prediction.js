...
myApp.controller("mainController", [
  ...
  function ($rootScope, $scope, $http, $window) {
    ...
    $scope.sendComment = function () {
      ...
      $http.get("/sendComment/" + toSendString).then(function (reponse) {
        ...
        if (inc.success) {
          ...
          $http
            .get("/commentData/" + reqPredictionString)
    ...
    $scope.deleteComment = function (id) {
      ...
      toSendObject.prediction = $scope.useGraphTitle;
      var toSendString = encodeURIComponent(JSON.stringify(toSendObject));
      $http.get("/deleteComment/" + toSendString).then(function (response) {
        var inc = response.data;
        var reqPredictionObject = {};
        reqPredictionObject.prediction = $scope.useGraphTitle;
        reqPredictionObject.group = $scope.currentGroup;
    ...
    $scope.upvote = function () {
      ...
      $http.get("/vote/" + toSendString).then(function (response) {
        var inc = response.data;
    ...
    $scope.update = function (localPrediction) {
      $scope.valueError = 0;
      if (
        0 <= localPrediction.val &&
        localPrediction.val <= 1 &&
        typeof localPrediction.val === "number"
      ) {
        ...
        $http
          .get("/sub/" + encodeURIComponent(JSON.stringify($scope.master)))
          .then(function (response) {
            var inc = response.data;
    ...
    $scope.getVote = function (string) {
      ...
      $http.get("/getVote/" + string).then(function (response) {
        var inc = response.data;
    ...
    $scope.drawGraph = function () {
      ...
      if ($scope.headline[headLineLocation] > -1) {
        $scope.headLineShowing =
          $scope.headline[headLineLocation].toFixed(4) * 100 + "%";
      }

      console.log("draw called");
      $scope.addColour();
      //var w = 700;
      var w = Math.min.apply(null, [
        700,
      ...
      var dateMax = new Date(
        ...
        highestDate.getMonth(),
        highestDate.getDate() + 2,
      );
      ...
      xGridAppend.selectAll(".grid path").style(gridPathStyle);
      xGridAppend
        .selectAll(".grid .tick")
        .attr("stroke", "0")
        .attr("opacity", "1");
      ...
      yGridAppend.selectAll(".grid path").style(gridPathStyle);
      yGridAppend.selectAll(".grid .tick").style(gridTickStyle);
      ...
      var yAxisAppend = svg
        ...
        .attr("transform", "translate(" + margin.left + ",0)")
        .call(yAxis);
      xAxisAppend.selectAll(".axis line, .axis path").style(axisStyle);
      yAxisAppend.selectAll(".axis line, .axis path").style(axisStyle);
      var nameCount = 0;
      ...
      for (name in predictionData) {
        ...
        console.log(
          "number of voters for bin...: " + Object.keys(predictionData).length,
        );
        if (acceptVoter) {
          ...
          for (j = 0; j < loopLength; j++) {
            if (j > 0) {
              toPush = [
                data[j - 1][0],
                new Date(new Date(data[j][1]).getTime() - 1),
              ];
              data.push(toPush);
            }
          }
