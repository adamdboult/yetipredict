...
module.exports = function (app, passport) {
  ...
  function renderJade(req, res, dest, renderHeader) {
    ...
    GroupSerie.findOne(
      { lName: group },
      "name open verbose admins members",
      function (err, idwa) {
        console.log(1);
        if (idwa) {
          privacy = idwa.open;
                    ...
                    User.findOneAndUpdate(
                      userFind,
                      {
                        $addToSet: {
                          ...
                          predictions:
                            findObject.group + "&" + findObject.ldesc,
                          groupsProper: groupProper,
                          predictionsProper:
                            findObject.group + "&" + result.desc,
                        },
                      },
                      { safe: true, upsert: true },
        ...
        if (tf) {
          GroupSerie.findOne(findObject, "members", function (err, idwa) {
            if (err) {
              ...
              idwa.members.indexOf(newName) === -1 &&
              newName !== "admin"
            ) {
              var findUser = {};
              ...
              if (err) {
                ...
                predictReceive.save(function (err, predictReceive) {
                  if (err) {
                    ...
                    var userFind = {};
                    userFind["local.username"] = makeLower(
          ...
          PredictSerie.findOne(findObject, function (err, foundPrediction) {
            ...
            if (outcomebase === newOutcome.outcome) {
              ...
              for (i = 0; i < foundPrediction.data.length; i++) {
                if (
                  affectedUsers.indexOf(foundPrediction.data[i].name) === -1 &&
                  foundPrediction.data[i].answer === answerDeclared
                ) {
              ...
              User.update(
                ...
                function (errors, results) {
                  User.find(userFind, "score outcomes local", { multi: true })
                    .stream()
    ...
    if (!reqObject) {
      ...
      isAdmin(req, group, function (tf, groupProper) {
        ...
        if (tf) {
          var findObject = {};
          findObject.group = group;
          PredictSerie.find(findObject, function (err0, idwa0) {
            var affectedUsers = [];
            var i, j, candidate;
  ...
  app.get("/deleteGraph/:id", function (req, res) {
    ...
    if (!reqObject) {
      ...
      predictionPermission(
        ...
        function (tf) {
          if (tf) {
            PredictSerie.findOne(findObject, function (error, inc) {
